...
import { Plus, Search, X, Archive, History, RefreshCcw, Filter, LogOut, Lock, ExternalLink, Edit, Trash2, Calendar, Wrench, CheckCircle, AlertTriangle } from 'lucide-react';
...
function App() {
  ...
  // --- ATUALIZAÇÃO: Motor de auto-scroll mágico ---
  const handleDragOver = (e) => {
    e.preventDefault();

    // Procura o container interno que tem a barra de rolagem (que acabamos de dar a classe .cartoes-container)
    const container = e.currentTarget.querySelector('.cartoes-container');
    if (container) {
      const rect = container.getBoundingClientRect();
      const y = e.clientY;
      const threshold = 80; // Área invisível nas bordas (80px) que ativa a rolagem

      if (y - rect.top < threshold) {
        container.scrollTop -= 15; // Rola pra cima
      } else if (rect.bottom - y < threshold) {
        container.scrollTop += 15; // Rola pra baixo
      }
    }
  };
      ...
      <div className="min-h-screen bg-white flex font-sans">
        ...
        >
          ...
          <div className="max-w-md text-center z-10">
            ...
            />
            <h1 className="text-4xl font-black text-white tracking-tight mb-4 uppercase">Portal de TI</h1>
        ...
        <div className="w-full lg:w-1/2 flex items-center justify-center p-8 md:p-16 lg:p-24 bg-white">
          <div className="w-full max-w-md">
            ...
            <div className="mb-10">
              ...
              <p className="text-slate-500 font-medium mt-1">Insira suas credenciais para acessar o painel.</p>
            </div>
            ...
            <form onSubmit={fazerLogin} className="flex flex-col gap-6">
              ...
              <div className="flex items-center justify-between mt-1">
                <div className="flex items-center gap-2">
                  ...
                  <label htmlFor="remember" className="text-sm font-semibold text-slate-600">Lembrar de mim</label>
                </div>
              ...
              <button type="submit" disabled={carregandoLogin} className="w-full bg-[#002f6c] text-white font-bold py-4 rounded-xl mt-4 hover:bg-[#001f4d] shadow-lg shadow-blue-900/20 transition-all flex justify-center items-center gap-2 text-lg">
                {carregandoLogin ? 'Autenticando...' : <><Lock className="w-5 h-5" /> Acessar Sistema</>}
              </button>
            </form>
  ...
  return (
    <div className="min-h-screen bg-[#f4f7f9] font-sans text-slate-800 relative flex flex-col">
      ...
      <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full flex-1 flex flex-col overflow-hidden">
        ...
        {/* --- ATUALIZAÇÃO: items-stretch para as colunas descerem até o fundo --- */}
          ...
          {colunas.map((coluna, index) => {
            ...
            return (
              <div key={index} className="flex-1 min-w-[320px] max-w-[400px] flex flex-col bg-slate-100/50 rounded-2xl p-4 border border-slate-200/60 max-h-full" onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, coluna.titulo)}>
                ...
                {/* --- ATUALIZAÇÃO: className cartoes-container para referenciar o scroll --- */}
                <div className="cartoes-container flex-1 flex flex-col gap-4 overflow-y-auto pr-2 pb-10">
                  {equipamentosDaColuna.length > 0 ? (
                    ...
                    <div className="flex items-center justify-center h-24 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50/50 mt-4"><span className="text-sm font-semibold text-slate-400">Solte o card aqui</span></div>
                  )}
                </div>
              </div>
            );
            ...
            <form onSubmit={modalEdicaoAberto ? salvarEdicao : salvarEquipamento} className="flex flex-col gap-4">
              ...
              {((modalEdicaoAberto && equipEditando.categoriaSelect === 'Outro') || (!modalEdicaoAberto && novaCategoria === 'Outro')) && (
                <div className="bg-blue-50 p-3 rounded-lg border border-blue-100">
                  ...
                  <input required type="text" value={modalEdicaoAberto ? equipEditando.categoriaOutro : novaCategoriaOutro} onChange={(e) => modalEdicaoAberto ? setEquipEditando({ ...equipEditando, categoriaOutro: e.target.value }) : setNovaCategoriaOutro(e.target.value)} className="w-full border-2 border-blue-200 p-2.5 rounded-lg font-bold text-slate-800 focus:border-[#002f6c] focus:outline-none transition-colors" placeholder="Ex: Mouse, CPU..." />
                </div>
              )}
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 mt-2">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    <div className="flex items-center justify-between">
                      <label className="text-sm font-bold text-slate-600">Cadastrado em</label>
